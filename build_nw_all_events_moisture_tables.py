# -*- coding: utf-8 -*-
"""
Northwest all-events moisture-support table builder
======================================================================

Purpose
-------
Build CSV tables for the second high-priority hard-mechanism diagnostic:

    Northwest all events grouped by recovered vs unrecovered
    tau = -5 ... 0
    RH, VIMD, CAPE, precipitation
...
Mechanism fields
----------------
- relative_humidity from monthly grid CSV files
- vertically_integrated_moisture_divergence_mean from monthly grid CSV files
- convective_available_potential_energy_mean from monthly grid CSV files
- precipitation from monthly grid CSV files

Outputs
-------
All outputs are written to a NEW directory:
    EVENT_ROOT/_nw_all_events_moisture_tables/

Main tables
-----------
- nw_all_events_moisture_event_catalog.csv
- nw_all_events_moisture_tau_event_box_means.csv
- nw_all_events_moisture_composite_grid_long.csv
- nw_all_events_moisture_composite_tau_summary.csv
- nw_all_events_moisture_group_counts.csv
- nw_all_events_moisture_run_info.csv

Notes
-----
- This script does not use netCDF4.
- Precipitation is written in mm if the source looks meter-based.
- Both raw VIMD and moisture convergence (-VIMD) are written for convenience.
- Unlike the matched builder, this version uses all Northwest events and groups them by recovered vs unrecovered.
"""
...
class Config:
    ...
    def __post_init__(self) -> None:
        if self.out_dir is None:
            self.out_dir = self.event_root / "_nw_moisture_mechanism_tables"
...
class CompositeAccumulator:
    ...
    sums: Optional[np.ndarray] = None  # shape=(4, lat, lon)
    counts: Optional[np.ndarray] = None

    def add(self, rh: xr.DataArray, vimd: xr.DataArray, cape: xr.DataArray, pr_mm: xr.DataArray) -> None:
        stack = np.stack([
            np.asarray(rh.values, dtype=float),
            np.asarray(vimd.values, dtype=float),
            np.asarray(cape.values, dtype=float),
            np.asarray(pr_mm.values, dtype=float),
        ], axis=0)
        if self.sums is None:
            self.lon = np.asarray(rh["lon"].values, dtype=float)
            self.lat = np.asarray(rh["lat"].values, dtype=float)
...
def build_nw_all_events_moisture_tables(cfg: Config) -> None:
    # 1) Reuse an existing Northwest event catalog if available; otherwise rebuild.
    summary = None
    if cfg.prefer_existing_nw_ridging_tables:
        summary_ridge, _pairs_unused = load_existing_nw_ridging_tables(cfg)
        if summary_ridge is not None:
            summary = summary_ridge
            print("[INFO] Reusing existing Northwest ridging tables for Northwest event catalog")

    if summary is None:
        summary_r3, _main_df_unused, _pairs_unused = (None, None, None)
        if cfg.prefer_existing_result3_tables:
            summary_r3, _main_df_unused, _pairs_unused = load_existing_result3_tables(cfg)
            if summary_r3 is not None:
                print("[INFO] Reusing existing Result 3 v3 tables for Northwest event catalog")
        if summary_r3 is not None:
            summary = summary_r3
        else:
            summary, _nw_unused, _pairs_unused = rebuild_event_catalog_and_matches(cfg)
    ...
    nw_catalog["group"] = np.where(to_num(nw_catalog.get("no_recovery", pd.Series(0, index=nw_catalog.index))).fillna(0).astype(int) == 1,
                                   "Unrecovered", "Recovered")
    nw_catalog.to_csv(cfg.out_dir / "nw_all_events_moisture_event_catalog.csv", index=False, encoding="utf-8-sig")
    grp_counts = nw_catalog.groupby("group", as_index=False).size().rename(columns={"size": "n_events"})
    grp_counts.to_csv(cfg.out_dir / "nw_all_events_moisture_group_counts.csv", index=False, encoding="utf-8-sig")

    # 2) Inventories
    capevimd_inv = build_monthly_inventory(
        cfg.capevimd_root,
        ["date", "convective_available_potential_energy_mean", "vertically_integrated_moisture_divergence_mean", "longitude", "latitude"],
    )
    rhpr_inv = build_monthly_inventory(
        cfg.rhpr_root,
        ["date", "relative_humidity", "precipitation", "longitude", "latitude"],
    )
    pr_scale, pr_note = infer_precip_scale_from_monthlies(rhpr_inv)
    print(f"[INFO] CAPE/VIMD monthly CSVs discovered: {len(capevimd_inv):,}")
    print(f"[INFO] RH/precip monthly CSVs discovered: {len(rhpr_inv):,} | precip note: {pr_note}")

    # 3) All Northwest event-date slices
    mean_rows = []
    ...
    for i, (event_uid, group, tau, date) in enumerate(all_dates, start=1):
        try:
            rh_da = get_date_field_from_inventory(rhpr_inv, date, "relative_humidity", cfg)
            lon_target = np.asarray(rh_da["lon"].values, dtype=float)
            lat_target = np.asarray(rh_da["lat"].values, dtype=float)
            pr_da = get_date_field_from_inventory(rhpr_inv, date, "precipitation", cfg, lon_target=lon_target, lat_target=lat_target)
            cape_da = get_date_field_from_inventory(capevimd_inv, date, "convective_available_potential_energy_mean", cfg, lon_target=lon_target, lat_target=lat_target)
            vimd_da = get_date_field_from_inventory(capevimd_inv, date, "vertically_integrated_moisture_divergence_mean", cfg, lon_target=lon_target, lat_target=lat_target)

            rh_vals = np.asarray(rh_da.values, dtype=float)
            pr_vals = np.asarray(pr_da.values, dtype=float) * pr_scale
            cape_vals = np.asarray(cape_da.values, dtype=float)
            vimd_vals = np.asarray(vimd_da.values, dtype=float)
            conv_vals = -vimd_vals
            ...
            mean_rows.append({
                ...
                "tau": tau,
                "date": date,
                "rh_mean": float(np.nanmean(rh_vals)),
                "vimd_mean": float(np.nanmean(vimd_vals)),
                "moisture_convergence_mean": float(np.nanmean(conv_vals)),
                "cape_mean": float(np.nanmean(cape_vals)),
                "precip_mm_mean": float(np.nanmean(pr_vals)),
                "rh_std": float(np.nanstd(rh_vals)),
                "vimd_std": float(np.nanstd(vimd_vals)),
                "cape_std": float(np.nanstd(cape_vals)),
                "precip_mm_std": float(np.nanstd(pr_vals)),
                "n_grid": int(np.isfinite(rh_vals).sum()),
            })
            ...
            composite[key].add(
                xr.DataArray(rh_vals, coords={"lat": lat_target, "lon": lon_target}, dims=("lat", "lon"), name="relative_humidity"),
                xr.DataArray(vimd_vals, coords={"lat": lat_target, "lon": lon_target}, dims=("lat", "lon"), name="vimd"),
                xr.DataArray(cape_vals, coords={"lat": lat_target, "lon": lon_target}, dims=("lat", "lon"), name="cape"),
                xr.DataArray(pr_vals, coords={"lat": lat_target, "lon": lon_target}, dims=("lat", "lon"), name="precip_mm"),
            )
        except Exception as e:
            n_fail += 1
            warnings.warn(f"Failed for {event_uid} tau={tau} date={date.date()}: {e}")
        if i % cfg.progress_every_dates == 0 or i == len(all_dates):
            print(f"[INFO] Extracted {i:,}/{len(all_dates):,} Northwest all-event slices")

    means = pd.DataFrame(mean_rows).sort_values(["group", "event_uid", "tau"]).reset_index(drop=True)
    means.to_csv(cfg.out_dir / "nw_all_events_moisture_tau_event_box_means.csv", index=False, encoding="utf-8-sig")
    ...
    for (group, tau), acc in sorted(composite.items(), key=lambda x: (x[0][0], x[0][1])):
        try:
            ...
            continue
        rh, vimd, cape, pr = mean_stack[0], mean_stack[1], mean_stack[2], mean_stack[3]
        conv = -vimd
        comp_summary_rows.append({
            "group": group,
            "tau": tau,
            "rh_mean": float(np.nanmean(rh)),
            "vimd_mean": float(np.nanmean(vimd)),
            "moisture_convergence_mean": float(np.nanmean(conv)),
            "cape_mean": float(np.nanmean(cape)),
            "precip_mm_mean": float(np.nanmean(pr)),
        ...
        for ii, lat in enumerate(acc.lat):
            for jj, lon in enumerate(acc.lon):
                comp_rows.append({
                    ...
                    "latitude": float(lat),
                    "longitude": float(lon),
                    "relative_humidity": float(rh[ii, jj]) if np.isfinite(rh[ii, jj]) else np.nan,
                    "vertically_integrated_moisture_divergence_mean": float(vimd[ii, jj]) if np.isfinite(vimd[ii, jj]) else np.nan,
                    "moisture_convergence": float(conv[ii, jj]) if np.isfinite(conv[ii, jj]) else np.nan,
                    "convective_available_potential_energy_mean": float(cape[ii, jj]) if np.isfinite(cape[ii, jj]) else np.nan,
                    "precip_mm": float(pr[ii, jj]) if np.isfinite(pr[ii, jj]) else np.nan,
                })

    pd.DataFrame(comp_rows).to_csv(cfg.out_dir / "nw_all_events_moisture_composite_grid_long.csv", index=False, encoding="utf-8-sig")
    pd.DataFrame(comp_summary_rows).to_csv(cfg.out_dir / "nw_all_events_moisture_composite_tau_summary.csv", index=False, encoding="utf-8-sig")
    ...
    info.to_csv(cfg.out_dir / "nw_all_events_moisture_run_info.csv", index=False, encoding="utf-8-sig")
    print(f"[DONE] Northwest all-events moisture tables written to: {cfg.out_dir}")
...
def main(cfg: Config = CFG) -> None:
    build_nw_all_events_moisture_tables(cfg)
