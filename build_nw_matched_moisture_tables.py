# -*- coding: utf-8 -*-
"""
Northwest matched recovered vs unrecovered moisture-support table builder
======================================================================
...
All outputs are written to a NEW directory:
    EVENT_ROOT/_nw_moisture_mechanism_tables/

Main tables
-----------
- nw_moisture_all_event_catalog.csv
- nw_moisture_matched_pairs_used.csv
- nw_moisture_matched_tau_event_box_means.csv
- nw_moisture_matched_composite_grid_long.csv
- nw_moisture_matched_composite_tau_summary.csv
- nw_moisture_run_info.csv
...
def build_nw_moisture_tables(cfg: Config) -> None:
    ...
    nw_catalog.to_csv(cfg.out_dir / "nw_moisture_all_event_catalog.csv", index=False, encoding="utf-8-sig")

    if pairs is None or pairs.empty:
        raise RuntimeError("No Northwest matched pairs available.")
    pairs = pairs.copy()
    pairs.to_csv(cfg.out_dir / "nw_moisture_matched_pairs_used.csv", index=False, encoding="utf-8-sig")
    ...
    rhpr_inv = build_monthly_inventory(
        cfg.rhpr_root,
        ["date", "precipitation", "relative_humidity", "longitude", "latitude"],
    )
    ...
    for i, (event_uid, group, tau, date) in enumerate(all_dates, start=1):
        try:
            # Use CAPE grid as reference target grid, then interpolate RH/precip onto it
            cape_da = get_date_field_from_inventory(
                capevimd_inv, date,
                "convective_available_potential_energy_mean",
                cfg,
            )
            lon_target = np.asarray(cape_da["lon"].values, dtype=float)
            lat_target = np.asarray(cape_da["lat"].values, dtype=float)
            vimd_da = get_date_field_from_inventory(
                capevimd_inv, date,
                "vertically_integrated_moisture_divergence_mean",
                cfg,
                lon_target=lon_target, lat_target=lat_target,
            )
            rh_da = get_date_field_from_inventory(
                rhpr_inv, date,
                "relative_humidity",
                cfg,
                lon_target=lon_target, lat_target=lat_target,
            )
            pr_da = get_date_field_from_inventory(
                rhpr_inv, date,
                "precipitation",
                cfg,
                lon_target=lon_target, lat_target=lat_target,
            ...
            mean_rows.append({
                ...
                "tau": tau,
                "date": date,
                "relative_humidity_mean": float(np.nanmean(rh_vals)),
                "vimd_mean": float(np.nanmean(vimd_vals)),
                "moisture_convergence_mean": float(np.nanmean(mc_vals)),
                "cape_mean": float(np.nanmean(cape_vals)),
                "precipitation_mm_mean": float(np.nanmean(pr_vals)),
                "relative_humidity_std": float(np.nanstd(rh_vals)),
                "vimd_std": float(np.nanstd(vimd_vals)),
                "cape_std": float(np.nanstd(cape_vals)),
                "precipitation_mm_std": float(np.nanstd(pr_vals)),
            ...
            composite[key].add(
                xr.DataArray(rh_vals, coords={"lat": lat_target, "lon": lon_target}, dims=("lat", "lon"), name="rh"),
                ...
                xr.DataArray(pr_vals, coords={"lat": lat_target, "lon": lon_target}, dims=("lat", "lon"), name="precipitation_mm"),
            )
        except Exception as e:
            n_fail += 1
    ...
    means.to_csv(cfg.out_dir / "nw_moisture_matched_tau_event_box_means.csv", index=False, encoding="utf-8-sig")
    ...
    for (group, tau), acc in sorted(composite.items(), key=lambda x: (x[0][0], x[0][1])):
        try:
            ...
            continue
        rh, vimd, cape, pr_mm = mean_stack[0], mean_stack[1], mean_stack[2], mean_stack[3]
        mc = -vimd
        comp_summary_rows.append({
            "group": group,
            "tau": tau,
            "relative_humidity_mean": float(np.nanmean(rh)),
            "vimd_mean": float(np.nanmean(vimd)),
            "moisture_convergence_mean": float(np.nanmean(mc)),
            "cape_mean": float(np.nanmean(cape)),
            "precipitation_mm_mean": float(np.nanmean(pr_mm)),
        ...
        for ii, lat in enumerate(acc.lat):
            for jj, lon in enumerate(acc.lon):
                comp_rows.append({
                    ...
                    "vimd": float(vimd[ii, jj]) if np.isfinite(vimd[ii, jj]) else np.nan,
                    "moisture_convergence": float(mc[ii, jj]) if np.isfinite(mc[ii, jj]) else np.nan,
                    "cape": float(cape[ii, jj]) if np.isfinite(cape[ii, jj]) else np.nan,
                    "precipitation_mm": float(pr_mm[ii, jj]) if np.isfinite(pr_mm[ii, jj]) else np.nan,
                })

    pd.DataFrame(comp_rows).to_csv(cfg.out_dir / "nw_moisture_matched_composite_grid_long.csv", index=False, encoding="utf-8-sig")
    pd.DataFrame(comp_summary_rows).to_csv(cfg.out_dir / "nw_moisture_matched_composite_tau_summary.csv", index=False, encoding="utf-8-sig")
    ...
    info.to_csv(cfg.out_dir / "nw_moisture_run_info.csv", index=False, encoding="utf-8-sig")
    print(f"[DONE] Northwest moisture-support mechanism tables written to: {cfg.out_dir}")
